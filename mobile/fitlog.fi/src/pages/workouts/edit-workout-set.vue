<template>
    <q-modal ref="modal" :class="{desktop: isDesktop }">
      ...
      <div class="row pad" v-if="!selectExercise">     
        <q-btn @click="reselectExercise" :label="exercise.name"></q-btn>
      </div>
      <div class="row pad" v-if="!selectExercise">     
      ...
      <div class="row pad buttons">
        <q-btn @click="close" :label="$t('cancel')"></q-btn>
        <q-btn color="primary" @click="save" v-if="exercise && quantity" :label="$t('save')"></q-btn>
      </div>
    </q-modal>
</template>
