    ...
    <q-tabs v-model="tab">
      ...
      <q-scroll-area style="height: 66vh;">
          <q-tab-pane v-for="(workout, index) in workouts" :name="'tab-' + index" :key="index">
            ...
            <q-card v-for="(group, index) in workout.groups" :key="index" class="q-ma-sm">
                ...
                <q-card-actions align="end">
                    <q-btn color="primary" icon="fa-plus" small glossy @click="addRow(group)" :label="$t('set')"></q-btn>
                </q-card-actions>
            </q-card>
            <div class="row pad buttons">
                <q-btn glossy color="primary" icon="fa-plus" small @click="addGroup(workout)" :label="$t('exercise')"></q-btn>
            </div>  
          </q-tab-pane>
      </q-scroll-area>
