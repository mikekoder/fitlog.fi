...
WHERE Feedback.Type=@type";
        ...
        public bool CreateFeedback(Feedback feedback)
        {
            ...
            try
            {
                ...
                conn.Execute(@"INSERT INTO FeedbackVote(FeedbackId,UserId,Time) VALUES (@FeedbackId,@UserId,@Time)", new { FeedbackId = feedback.Id, feedback.UserId, Time = feedback.Created }, tran);
                tran.Commit();
                return true;
            }
            catch
        ...
        public bool DeleteFeedback(Feedback feedback)
        {
            ...
            try
            {
                conn.Execute("UPDATE Feedback SET Deleted=@Deleted WHERE Id=@Id", new { feedback.Id, Deleted = DateTimeOffset.Now }, tran);
                tran.Commit();
                return true;
            }
            catch
...
SELECT * FROM FeedbackComment WHERE FeedbackId=@Id";
        ...
        public bool UpdateFeedback(Feedback feedback)
        {
            ...
            try
            {
                conn.Execute("UPDATE Feedback SET Title=@Title,Content=@Content WHERE Id=@Id", new { feedback.Id, Deleted = DateTimeOffset.Now }, tran);
                tran.Commit();
                return true;
            }
            catch
