...
namespace Fitlog.Web.Controllers
{
    ...
    public class UsersController : ApiControllerBase
    {
        ...
        [AllowAnonymous]
        public IActionResult ExternalLogin(string provider, string returnUrl = null, string client = null, string token = null)
        {
...
Salasanan pääset nollaamaan <a href=""https://fitlog.fi/#/vaihda-salasana/{user.Id}/{HttpUtility.UrlEncode(token)}"">tästä linkistä</a>.
        ...
        private async Task<IActionResult> LoginWithGoogleToken(string idToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(refreshToken))
            {
                _profileRepository.UpdateRefreshToken(user.Id);
            }
